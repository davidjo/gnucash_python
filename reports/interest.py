...
class Interest(ReportTemplate):
    ...
    def __init__ (self):
        ...
        self.version = 1
        self.name = N_("Interest")
        self.report_guid = "ac56181418534f85a4907a14c5a670cf"
    ...
    def account_in_list (acnt,acclst):
        for acc in acclst:
            if Interest.same_account(acnt,acc):
                return True
        return False
    ...
    def account_in_alist (acnt,alst):
        for acpr in alst:
            ...
            if Interest.same_account(acnt,acpr[0]):
                return acpr
        return None
    ...
    def renderer (self, report):
        ...
        if len(accounts) > 0:
            ...
            # find interest accounts - junkily just look for name
            ...
            for acc in accounts:
                if acc.get_current_depth() <= tree_depth:
                    accnm = acc.GetName()
                    if accnm.lower().find('interest') >= 0:
                        if not Interest.account_in_list(acc,interest_accounts):
                            interest_accounts.append(acc)
                            if shwopt:
                                subacclst = get_all_subaccounts([acc])
                                for subacc in subacclst:
                                    if not Interest.account_in_list(subacc,interest_accounts):
                                        interest_accounts.append(subacc)
                    else:
                        parent = acc.get_parent()
                        prntnm = parent.GetName()
                        if prntnm.lower().find('interest') >= 0:
                            if not Interest.account_in_list(acc,interest_accounts):
                                interest_accounts.append(acc)
                                if shwopt:
                                    subacclst = get_all_subaccounts([acc])
            ...
            work_done = 0
            work_to_do = len(interest_accounts)
	    ...
	    new_data = document.doc.SubElement(new_row,"td",attrib={'rowspan' : "1", 'colspan' : "2" })
            ...
            new_col.text = N_("Interest Account")

            new_col = document.doc.SubElement(new_row,"td")
            new_col.text = N_("Payment Date")

            new_col = document.doc.SubElement(new_row,"td")
            new_col.text = N_("Interest Income")
