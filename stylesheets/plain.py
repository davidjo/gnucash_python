...
class Plain(StylesheetTemplate):
    ...
    def renderer (self):
        ...
        update(ssdoc.style_table,{"body" : { 'attribute' : { "bgcolor" : clropt.get_value_as_html() } } })
        update(ssdoc.style_table,{"body" : { 'attribute' : { "bgcolor" : clropt.get_value_as_html() } } })
        ...
        update(ssdoc.style_table,{"table" : \
                   { 'attribute' : { "border" : str(bdropt.get_value()),
                                     "cellcpacing" : str(spcopt.get_value()),
        ...
        if ssdoc.doc.engine_supports_css:
            ...
            update(ssdoc.style_table,{"column-heading-left" : { "tag" : "th", "attribute" : { "class" : "column-heading-left" }}})

        else:
            # and this is the old gtkhtml
            pass

        update(ssdoc.style_table,{"normal-row" : { "tag" : "tr" } })

        altopt = self.options.lookup_name('Colors','Alternate Table Cell Color')
        update(ssdoc.style_table,{"alternate-row" : { "tag" : "tr" , "attribute" : { "bgcolor" : altopt.get_value_as_html() } } })

        update(ssdoc.style_table,{"primary-subheading" : { "tag" : "tr" , "attribute" : { "bgcolor" : clropt.get_value_as_html() } } })
        update(ssdoc.style_table,{"secondary-subheading" : { "tag" : "tr" , "attribute" : { "bgcolor" : clropt.get_value_as_html() } } })
        update(ssdoc.style_table,{"grand-total" : { "tag" : "tr" , "attribute" : { "bgcolor" : clropt.get_value_as_html() } } })
        ...
        if not dolinks:
            update(ssdoc.style_table,{"a" : { "tag" : "" }})
        ...
        # in the scheme way the title and objects are copied from 
        # the passed document
        #ssdoc.title = N_("Hello, World")
